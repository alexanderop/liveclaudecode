...
/**
 * Where each harness stores its transcripts. Five frozen strings that no test
 * changes, so the block builds them once through `it.layer`.
 *
 * Everything stateful is deliberately left out: the five caches below and the
 * in-memory filesystem are rebuilt for every case, because `it.layer` memoizes
 * for the whole block and a shared cache would let one test's scans answer the
 * next test's lookups.
 */
