...
import { operationConcurrencyProbe, testFileSystem } from '../fixtures/filesystem'
 ...
 *
 * They hold two strings and nothing else, so the whole block can share one
 * construction via `it.layer`. The filesystem stays per test: each case brings
 * its own tree, and `testFileSystem` also carries the discovery limiter's
 * permit pool, which the concurrency case below must not share with anyone.
 */
...
describe('project resolution', () => {
  it.layer(references)('with the standard project references', (it) => {
    ...
    it.effect('accepts a transcript directory directly', () =>
      resolveProjectDirectory('/home/me/transcripts').pipe(
        ...
        Effect.provide(testFileSystem({ '/home/me/transcripts/run.jsonl': '{}\n' })),
      ))
    ...
    it.effect('resolves a slug under the projects directory', () =>
      resolveProjectDirectory('my-project').pipe(
        ...
        Effect.provide(testFileSystem({ [`${PROJECTS}/my-project/run.jsonl`]: '{}\n' })),
      ))
    ...
    it.effect('reports an unknown project as a typed failure', () =>
      ...
      }).pipe(Effect.provide(testFileSystem({ [`${PROJECTS}/first/one.jsonl`]: '{}\n' }))))
